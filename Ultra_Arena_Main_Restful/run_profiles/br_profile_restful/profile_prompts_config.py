...
USER_PROMPT = """
    ...
    * **'INVOICE_NO'**: O valor único para 'INVOICE_NO' deve ser extraído no início do arquivo, geralmente após algum dos textos a seguir, independentemente das variações de maiúsculas/minúsculas, acentos ou formatação:
        - "NFS-e", "NFSe", "NF-e", "NFe", "Nota Fiscal de Serviço", "Nota Fiscal de Serviços", "Nota Fiscal", "Nota", "Número da Nota Fiscal", "Número da Nota", "Número NF", "Número da NF", "Nº da Nota", "Nº da Nota Fiscal", "Nº", "Num. da Nota", "Num da NF", "Num NF", 
      Considere variações na ordem, abreviações, dígitos ou pontuações nestes textos.
      O número da nota fiscal pode ser um número inteiro de qualquer comprimento – NÃO restringir ao intervalo típico nem ao tamanho (ex: pode ser apenas "1870" ou um número longo como "2025000000980"). Em alguns casos esse número pode vir precedido pelo ano (ex: "20242000001310") ou juntos sem espaços ou caracteres intermediários ao texto. 
      **CRÍTICO:** Nunca extraia números que vêm depois de textos como "Número RPS", ou semelhantes, (Ex.:Número RPS, RPS, NRPS, Nº RPS, etc), pois desejamos APENAS o número da nota fiscal, e não outros números como RPS ou referências semelhantes.
      Sempre extraia APENAS o número associado ao texto identificado e que aparece no início do arquivo/documento (evite capturar números que aparecem após blocos secundários do texto).
    * **'INVOICE_ISSUE_DATE'**: O valor único para 'INVOICE_ISSUE_DATE' está localizado no início do arquivo, normalmente após o texto 'Emitida em' ou semelhante. Este valor **SEMPRE** possui formato de data brasileira, com ''dia'/'mês'/'ano'', no formato "DD/MM/AAAA, onde DD, MM, AAAA representam o dia, mês e ano, respectivamente". **IMPORTANTE**: Extraia APENAS o valor que está REALMENTE presente no documento. NÃO invente ou copie valores de exemplos.
