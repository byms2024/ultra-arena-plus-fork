...
class TogetherAIStyledStreamingClient(BaseLLMClient):
    ...
    def _parse_response(self, response: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            # Handle TogetherAI's response for multi-parts request format with "results" or "result" key
            if isinstance(parsed, dict) and "results" in parsed and isinstance(parsed["results"], list):
                ...
                logging.info(f"🔍 Detected TogetherAI batch response with {len(parsed['result'])} results")
